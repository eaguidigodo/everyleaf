...
RSpec.describe 'Task model function', type: :model do
  ...
  describe 'Search function' do
    ...
    context 'Label is performed by scope method' do
      it "Tasks containing search label keywords are narrowed down" do
        # title_seach is a Title search method presented by scope. The method name can be arbitrary.
        expect(Task.label_search('green')).to include(second_task)
        expect(Task.label_search('yellow')).not_to include(task)
        expect(Task.label_search('red').count).to eq 2
      end
    end
